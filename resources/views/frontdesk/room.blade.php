    ...
    <tr class="roomNumberRow">
        ...
        @for ($i = $startDay; $i < $startDay+$num_of_day; $i++) @foreach($guests as $guest) @if($guest->month==$mon && $guest->year==$year)
            ...
            @for($i=$guest->start_day;$i<=$guest->end_day;$i++)
                ...
                <td title=" အခန်းကြိုယူထား" id="reserv" class=" reserv" colspan="{{$guest->end_day-$guest->start_day+1}}">
                ...
                <td title="ဧည့်ထွက်" class="guest_cell checkout" colspan="{{$guest->end_day-$guest->start_day+1}}">
                    ...
                    <p class="my-0 guest_name">
                        ...
                        <span class="text-hide ">
                            {{$guest->phone}}&nbsp;{{$guest->start_day}}&nbsp;{{$guest->end_day}}&nbsp;{{$type->price}}&nbsp;{{$room->roomtype}}
                        </span>
                    </p>
                </td>
